...
function RouteComponent() {
  ...
  const referencesQuery = useFetchReferences(queryParams, ENDPOINTS.screening);
  ...
  const page = useScreeningPage(
    ...
    {
      pageTitle: 'Screening',
      defaultLayout: 'title-abstract',
      opinionStage: 'screening',
      endpoint: ENDPOINTS.screening,
      exportFilename: (id, filtered) =>
        `review-${id}-screening${filtered ? '-filtered' : ''}.bib`,
    },
    filters,
    references
  );
  ...
  return (
    <ReferenceTableLayout
      ...
      onExport={(type) => page.handleExport(type, exportScreening, queryParams)}
