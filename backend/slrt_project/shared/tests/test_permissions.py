"""
Tests for slrt_project/shared/permissions.py.
...
PERMISSIONS map
  - every Permission has an entry
  - role lists are non-empty

get_user_role
  - returns the correct Role for a member
  - returns None when the user is not a member

can
  - returns True when the user's role is allowed
  - returns False when the user's role is not allowed
  - returns False when the user is not a member of the review

humanize_permission
  - non-modify permissions are returned as plain words
  - modify permissions have "the" inserted

permission_denied_message
  - message contains "You cannot"
  - message lists the allowed roles

check_permission
  - does not raise when the user has permission
  - raises PermissionDenied when the user lacks permission
  - raised exception message is human-readable

IsReviewOwner
  - has_object_permission returns True for owners
  - has_object_permission returns False for non-owners
  - has_object_permission returns False for non-members
"""
