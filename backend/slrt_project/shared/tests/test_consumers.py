"""
Tests for slrt_project/reviews/consumers.py.

Coverage
--------
TaskStatusConsumer
  - connect joins the correct channel-layer group and sends initial status
  - _get_task_status returns the right payload for each Celery state
  - task_status_update forwards data and closes on terminal states
  - disconnect leaves the group

AuthenticateReviewMemberMixin
  - authenticate sets review_id / member on success
  - authenticate closes with 4003 when user is not a member
  - authenticate closes with 4000 on unexpected exception

ReviewGroupConsumer
  - connect sends message_history to the client
  - disconnect leaves the group
  - handle_chat_message rejects empty messages
  - handle_chat_message rejects over-length messages
  - handle_chat_message persists and broadcasts valid messages
  - save_chat_message failure sends an error frame
  - receive_json dispatches by type; unknown type is logged
  - handle_typing broadcasts a user_typing event
  - user_typing suppresses events from the connection's own user
  - chat_message channel handler forwards all fields

ScreeningStatConsumer
  - connect registers in cache, increments session on first tab
  - connect does NOT increment session for a second tab
  - disconnect removes from cache and saves in-progress session
  - disconnect cleans up cache when last tab closes
  - receive_json heartbeat resets idle timer and sends ack
  - receive_json start_tracking starts tracking when not on break
  - receive_json start_tracking is ignored while on break
  - receive_json stop_tracking stops tracking
  - receive_json break_start stops tracking and sets is_on_break
  - receive_json break_end resumes tracking and clears is_on_break
  - _save_current_session discards sessions shorter than 5 seconds
  - _save_current_session saves sessions >= 5 seconds
  - _check_heartbeat closes socket after MAX_IDLE_TIME
  - _increment_session creates ScreeningStat if absent and increments
  - _update_stats adds seconds to existing stat

All consumers are tested by constructing instances directly and replacing
``send_json``, ``close``, and ``channel_layer`` with ``AsyncMock`` / ``MagicMock``.
This avoids any dependency on daphne or a live channel layer.
"""
